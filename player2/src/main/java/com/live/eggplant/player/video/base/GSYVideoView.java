...
import com.live.eggplant.player.listener.GSYMediaPlayerListener;
import com.live.eggplant.player.listener.VideoAllCallBack;
import com.live.eggplant.player.utils.CommonUtil;
import com.live.eggplant.player.utils.Debuger;
import com.live.eggplant.player.utils.NetInfoModule;
...
import static com.live.eggplant.player.utils.CommonUtil.getTextSpeed;
...
public abstract class GSYVideoView extends GSYTextureRenderView implements GSYMediaPlayerListener {
    ...
    //http request header
    protected Map<String, String> mMapHeadData = getMapHeadData();
    ...
    protected void showPauseCover() {
        if (mCurrentState == CURRENT_STATE_PAUSE && mFullPauseBitmap != null
            ...
            if (getGSYVideoManager().isSurfaceSupportLockCanvas()) {
                try {
                    RectF rectF = new RectF(0, 0, mTextureView.getWidth(),
                            mTextureView.getHeight());
                    Canvas canvas = mSurface.lockCanvas(new Rect(0, 0, mTextureView.getWidth(),
                            mTextureView.getHeight()));
                    if (canvas != null) {
    ...
    protected Context getActivityContext() {
        return new WeakReference<>(CommonUtil.getActivityContext(getContext())).get();
    }
    ...
    protected void init(Context context) {
        ...
        mAudioManager = getmAudioManager();

    }

    public AudioManager getmAudioManager() {//TODO 内存优化

        if (mAudioManager == null) {
            mAudioManager =
                    new WeakReference<>((AudioManager) new SoftReference<>(getActivityContext()).get().getApplicationContext().getSystemService(Context.AUDIO_SERVICE)).get();
        }
        ...
        try {
            ...
            if (e.toString().contains("GSYImageCover")) {
                Debuger.printfError("********************\n" +
                        ...
                        "********************\n");
                e.printStackTrace();
                throw new InflateException("该版本需要清除布局文件中的GSYImageCover，please remove " +
                        "GSYImageCover from your layout");
            } else {
                e.printStackTrace();
            }
        }
    ...
    protected void startPrepare() {
        ...
        getmAudioManager().requestAudioFocus(onAudioFocusChangeListener,
                AudioManager.STREAM_MUSIC, AudioManager.AUDIOFOCUS_GAIN_TRANSIENT);
        ((Activity) getActivityContext()).getWindow().addFlags(WindowManager.LayoutParams.FLAG_KEEP_SCREEN_ON);
        mBackUpPlayingBufferState = -1;
        // getGSYVideoManager().prepare(mUrl, (mMapHeadData == null) ? new HashMap<String,
        // String>() : mMapHeadData, mLooping, mSpeed, mCache, mCachePath, mOverrideExtension);
        getGSYVideoManager().prepare(mUrl, (mMapHeadData == null) ? getMapHeadData() :
                mMapHeadData, mLooping, mSpeed, mCache, mCachePath, mOverrideExtension);
    ...
    protected AudioManager.OnAudioFocusChangeListener onAudioFocusChangeListener =
            new AudioManager.OnAudioFocusChangeListener() {
                ...
                public void onAudioFocusChange(int focusChange) {
                    // onAudioManagerFocusChange(focusChange);
                    switch (focusChange) {
    ...
    public boolean setUp(String url, boolean cacheWithPlay, File cachePath,
        ...
        if (setUp(url, cacheWithPlay, cachePath, title)) {
            if (this.mMapHeadData != null) {
                ...
                this.mMapHeadData = getMapHeadData();
            }
            if (mapHeadData != null) {
    ...
    public void onError(int what, int extra) {
        ...
        if (mNetChanged) {
            ...
            netWorkErrorLogic();
            if (mVideoAllCallBack != null) {
                mVideoAllCallBack.onPlayError(what, mOriginUrl, mTitle, this);
            }
            return;
        }
    ...
    protected void createNetWorkState() {
        if (mNetInfoModule == null) {
            mNetInfoModule = getNetInfoModule();
            mNetSate = mNetInfoModule.getCurrentConnectionType();
        }
    }

    private NetInfoModule getNetInfoModule() {
        if (mNetInfoModule == null) {
            mNetInfoModule =
                    new WeakReference<>(new NetInfoModule(getActivityContext().getApplicationContext(), new WeakReference<>(new NetInfoModule.NetChangeListener() {
                        @Override
    ...
    protected void listenerNetWorkState() {
        if (getNetInfoModule() != null) {
            getNetInfoModule().onHostResume();
        }
    }
    ...
    public void releaseNetWorkState() {
        Log.e("CustomGSYVideoPlayer2", "releaseNetWorkState");
        if (mNetInfoModule != null) {
            Log.e("CustomGSYVideoPlayer2", "releaseNetWorkStatem NetInfoModule != null");
    ...
    public Map<String, String> getMapHeadData() {
        ...
        if (mMapHeadData == null) {
            //   mMapHeadData = new WeakReference<>(new HashMap<String, String>()).get();
            //   new ArrayMap<>();
            if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.KITKAT) {
                mMapHeadData = new WeakReference<>(new ArrayMap<String, String>()).get();
            } else {
                mMapHeadData = new WeakReference<>(new TreeMap<String, String>()).get();
            }

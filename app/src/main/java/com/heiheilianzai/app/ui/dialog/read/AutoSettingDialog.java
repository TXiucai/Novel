    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setOnDismissListener(new OnDismissListener() {
            ...
            public void onDismiss(DialogInterface dialog) {
                ...
                AutoProgressBar.getInstance().setTime(ReadingConfig.getInstance().getAutoSpeed() * 1000);
                if (isChanged) {
                    AutoProgressBar.getInstance().restart();
                } else {
                    AutoProgressBar.getInstance().goStill();
                }
                isChanged = false;
                hideSystemUI();
            }
    ...
    public void onClick(View view) {
        switch (view.getId()) {
            case R.id.auto_setting_exit:
                if (AutoProgressBar.getInstance().isStarted() && !AutoProgressBar.getInstance().isStop()) {
